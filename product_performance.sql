...
-- Category revenue (USING WINDOW FUNCTION)
SELECT 
    c.category_name, 
    ROUND(SUM(total_revenue.revenue), 2) AS revenue_totals,
    ROUND(SUM(total_revenue.revenue) / SUM(SUM(total_revenue.revenue)) OVER () * 100, 2) AS revenue_percentage
FROM (
    SELECT 
        p.category_id, 
