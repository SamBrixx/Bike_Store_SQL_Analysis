...
-- Creating a new column to show Revenue percentage by using CTE Window Functions
SELECT 
    s.store_name, 
    ...
    ROUND(SUM(oi.quantity * oi.list_price * (1 - COALESCE(oi.discount, 0))) / 
          SUM(SUM(oi.quantity * oi.list_price * (1 - COALESCE(oi.discount, 0)))) OVER () * 100, 2) AS revenue_percentage
FROM orders AS o
INNER JOIN order_items AS oi USING (order_id)
INNER JOIN stores AS s USING (store_id)
GROUP BY s.store_name
